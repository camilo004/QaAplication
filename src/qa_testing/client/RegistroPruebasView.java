	...
	private void createMetricasWindow() {
		...
		aplicativoItem.addChangedHandler(new ChangedHandler() {
			...
			public void onChanged(ChangedEvent event) {
				...
				if (selectedValue != null && !selectedValue.isEmpty()) {
					...
					// Limpiar las opciones del ComboBoxItem de versiones si no se ha seleccionado
					// un aplicativo
					versionItem.clearValue();
				}
			}
		...
		guardarButton.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				...
				databaseService.insertVersionAndTestCases(versionId, cicloPrueba, casosPrueba, porcentajeExitosos,
						errores, tiempoPromedio, tasaFallos, new AsyncCallback<Boolean>() {
							@Override
							public void onSuccess(Boolean result) {
		...
		metricasForm.setFields(aplicativoItem, versionItem, ciclosItem, casosPruebaItem, porcentajeExitososItem,
				erroresItem, tiempoPromedioItem, tasaFallosItem);
